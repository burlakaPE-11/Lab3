...
double F(double x)
{
    return pow(x,3)-2*pow(x,2)+2*x+2;
}

void Fun(double xi,double fault,int n,double Timer)
{
    printf("xi = %lf\n",xi);
    printf("Fault  = %lf\n",fault);
...
int main()
{
    //My function y=pow(x,3)-2*pow(x,2)+2*x+2
    ...
    case 1:
      ...
      clock_t T2 = clock();
        Fun(xi,fabs (F(xi)),n,(double)(T2 - T1) / CLOCKS_PER_SEC);
       break;
    case 2:
      do
      {
      ...
      while ( fabs (F(xi)) > fault );
      clock_t T22 = clock();
      Fun(xi,fabs (F(xi)),n,(double)(T22 - T11) / CLOCKS_PER_SEC);
      break;
      }
